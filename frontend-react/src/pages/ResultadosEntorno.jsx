...
function ResultadosEntorno() {
  ...
  const handleDescargarPDF = () => {
    ...
    try {
      ...
      const titleText = 'REPORTE DE RESULTADOS - ENTORNO ORGANIZACIONAL'
      ...
      addText(doc, `Total Encuestas: ${resumen?.progreso || '0/0'}`, 110, y)
      y += 7
      addText(doc, `Puntaje Promedio: ${resumen?.puntajePromedio || 0}`, 15, y)
      addText(doc, `Nivel de Riesgo: ${resumen?.nivelRiesgo || 'Nulo'}`, 110, y)
      y += 7
      addText(doc, `Fecha de Actualización: ${formatDate(resumen?.ultimaActualizacion)}`, 15, y)
      ...
      respuestas.forEach((respuesta, idx) => {
        ...
        if (respuesta.puntajesPorCategoria && Object.keys(respuesta.puntajesPorCategoria).length > 0) {
          // Encabezado de tabla
          ...
          Object.entries(respuesta.puntajesPorCategoria).forEach(([categoria, puntaje]) => {
            ...
            addText(doc, categoria, 18, y + 4.5)
            addText(doc, String(puntaje), 160, y + 4.5)
            y += 7
          })
        } else {
        ...
        if (respuesta.puntajesPorDominio && Object.keys(respuesta.puntajesPorDominio).length > 0) {
          // Encabezado de tabla
          drawRect(15, y, 180, 6, true)
          doc.setFontSize(9)
          doc.setFont(undefined, 'bold')
  ...
  const handleDescargarExcel = () => {
    ...
    try {
      ...
      const respuestasData = [
        ['ID Encuesta', 'Fecha', 'Puntaje Total', 'Nivel de Riesgo', 'Recomendación', 'Es Jefe', 'Servicio a Clientes']
      ]
      ...
      respuestas.forEach((respuesta, idx) => {
        ...
        if (Object.keys(puntajesPorCategoria).length === 0) {
          categoriasData.push([
            encuestaId,
            fecha,
            'Sin datos',
      ...
      respuestas.forEach((respuesta, idx) => {
        ...
        if (Object.keys(puntajesPorDominio).length === 0) {
          dominiosData.push([
            encuestaId,
            fecha,
            'Sin datos',
